...
function tokenIsValid(token) {
  const secret = getSecret();
  const nextSecret = process.env.NETWORK_CRM_AUTOMATION_TOKEN_NEXT || "";
  return (!!secret && safeEqual(token, secret))
    || (!!nextSecret && safeEqual(token, nextSecret));
}

function messagesTokenIsValid(token) {
  const messagesSecret = process.env.NETWORK_CRM_MESSAGES_TOKEN || "";
  return (!!messagesSecret && safeEqual(token, messagesSecret)) || tokenIsValid(token);
}
