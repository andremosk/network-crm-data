...
async function ensureSchema(sql = getSql()) {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS crm_deleted_records (
      record_type TEXT NOT NULL CHECK (record_type IN ('contact', 'application', 'engagement')),
      record_id TEXT NOT NULL,
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS crm_deleted_records_deleted_at_idx ON crm_deleted_records (deleted_at DESC)`;
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS crm_gmail_review_sync_state (
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS crm_gmail_email_enrichment_state (
      mailbox_email TEXT PRIMARY KEY,
  ...
  // Stores automation idempotency results. Contact records also carry a small
  // request marker so a retry remains safe if a response is interrupted.
  await sql`
    CREATE TABLE IF NOT EXISTS crm_automation_write_requests (
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS crm_automation_write_requests_updated_at_idx ON crm_automation_write_requests (updated_at DESC)`;
}
