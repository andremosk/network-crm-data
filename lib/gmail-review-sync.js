...
const DEFAULT_REVIEW_BATCH_SIZE = 25;
const DEFAULT_ENRICHMENT_BATCH_SIZE = 20;
...
async function runGmailReviewSync({ sql, env = process.env, fetchImpl = fetch, now = new Date(), dependencies = {} }) {
  ...
  const batchSize = Math.max(1, Math.min(Number(env.NETWORK_CRM_GMAIL_REVIEW_BATCH_SIZE) || DEFAULT_REVIEW_BATCH_SIZE, 100));
  const since = state?.last_successful_at || new Date(now.getTime() - lookbackDays * 86400000).toISOString();
  const list = await (dependencies.listMessages || (async (token, date) => gmailRequest(token, `/messages?labelIds=INBOX&q=${encodeURIComponent(`after:${dateQuery(date)} -from:me`)}&maxResults=${batchSize}`, fetchImpl)))(accessToken, since);
...
async function runGmailEmailEnrichment({ sql, env = process.env, fetchImpl = fetch, dependencies = {} }) {
  ...
  const batchSize = Math.max(1, Math.min(Number(env.NETWORK_CRM_GMAIL_ENRICHMENT_BATCH_SIZE) || DEFAULT_ENRICHMENT_BATCH_SIZE, 100));
  const list = await (dependencies.listMessages || (async (token, pageToken) => gmailRequest(token, `/messages?includeSpamTrash=false&maxResults=${batchSize}&q=${encodeURIComponent("in:anywhere")}${pageToken ? `&pageToken=${encodeURIComponent(pageToken)}` : ""}`, fetchImpl)))(accessToken, state?.next_page_token || "");
  const messages = Array.isArray(list.messages) ? list.messages : [];
