...
function analyzeOutboundEmail(payload, matchedContact = null, now = new Date()) {
  ...
  const sentAt = new Date(payload.sentAt || payload.occurredAt || now);
  const recipients = recipientList(payload);
  const subject = cleanText(payload.subject, 300);
  const body = cleanText(payload.bodyText || payload.text || payload.body);
...
function sanitizeProposal(type, proposed) {
  ...
  if (type === "update_contact") return { note: common.notes, status, followUpDate: common.followUpDate, lastContact: common.lastContact };
